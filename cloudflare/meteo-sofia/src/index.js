/* LOGNAVAK — Worker Cloudflare « meteo-sofia » : TEMSI France, TEMSI EUROC et
   WINTEM France depuis SOFIA-Briefing, PDF vérifié servi avec CORS.

   POURQUOI CE WORKER : Météo-France refuse les octets aux adresses IP de
   centres de données que le relais Supabase emprunte (« Request forbidden by
   administrative rules »). La note technique du 26/08/2026 a constaté que la
   même récupération ABOUTIT depuis un Worker Cloudflare. Ce fichier est la
   coque HTTP posée sur le client de poc/temsi_wintem/meteo_client.js —
   la même séquence, les mêmes garde-fous, ÉPROUVÉS par 73 assertions contre
   une doublure stricte (poc/temsi_wintem/test_meteo.js). En cas d'évolution,
   modifier le client LÀ-BAS, rejouer son banc, puis recopier ici (le corps du
   fichier, jusqu'à la coque HTTP, est ce client à l'identique, moins son
   export CommonJS).

   Déploiement : voir LISEZMOI.md à côté. Aucun secret requis ; AEROWEB_ID
   (repli officiel Aéroweb) est optionnel. */
...
/* ══════════════════════════ LA COQUE HTTP DU WORKER ══════════════════════════

   Points d'accès (tous avec CORS ouvert) :

     GET /                                   -> ce que sert ce Worker
     GET /api/meteo/catalog?product=temsi-france|temsi-euroc|wintem-france
                           [&reference=ISO]  -> catalogue classé (JSON)
     GET /api/meteo/pdf?product=…[&valid=ISO][&reference=ISO]
         valid présent : cette échéance exacte, ou 404 avec les disponibles ;
         valid absent  : la carte de référence à « reference » (défaut :
         maintenant), sinon la prochaine publiée, sinon la dernière connue —
         la même règle que l'application.

   Variables (wrangler.toml [vars] ou secrets) :
     SOFIA_URL / METEO_URL : bases de remplacement (bancs d'essai) ;
     AEROWEB_ID            : code d'accès Aéroweb (repli officiel), optionnel.

   Le « login » des liens Météo-France ne sort JAMAIS d'ici : les catalogues
   rendus au client remplacent le lien par sa forme masquée. */

const VERSION_W = "meteo-sofia 1.0";

/* catalogue mémorisé 120 s par produit : l'application demande plusieurs
   cartes d'un coup, et le login des liens reste frais à cette échelle */
const memoCat = new Map();
async function catalogueMemo(produit, o) {
  /* la clé porte aussi les bases : en production elles sont constantes, mais
     un banc qui pointe plusieurs doublures ne doit pas lire le mémo d'une
     autre — et une erreur ne se mémorise jamais */
  const cle = produit + "|" + (o.sofia || "") + "|" + (o.meteo || "");
  const su = memoCat.get(cle);
  if (su && Date.now() - su.t < 120000) return su.cat;
  const cat = await catalogue(produit, o);
  memoCat.set(cle, { t: Date.now(), cat });
  return cat;
}
...
function json(obj, statut = 200) {
  return new Response(JSON.stringify(obj, null, 1), { status: statut,
    headers: Object.assign({ "Content-Type": "application/json; charset=utf-8",
      "Cache-Control": "no-store", "X-Worker-Version": VERSION_W }, ENTETES_CORS) });
}
/* un item pour l'extérieur : jamais le lien (il porte le login éphémère) */
function itemPublic(it) {
  if (!it) return null;
  const o = Object.assign({}, it);
  if (o.lien) { o.lienMasque = masque(o.lien); delete o.lien; }
  return o;
}

export default {
  async fetch(request, env) {
    if (request.method === "OPTIONS") return new Response(null, { status: 204, headers: ENTETES_CORS });
    const url = new URL(request.url);
    const o = { sofia: env && env.SOFIA_URL, meteo: env && env.METEO_URL,
      aerowebId: (env && env.AEROWEB_ID) || "" };
    try {
      if (url.pathname === "/api/meteo/catalog") {
        const produit = url.searchParams.get("product") || "";
        if (!PRODUITS[produit]) return json({ erreur: "produit inconnu : " + produit,
          produits: Object.keys(PRODUITS) }, 400);
        const ref = url.searchParams.get("reference") || new Date().toISOString();
        if (!isFinite(Date.parse(ref))) return json({ erreur: "reference invalide : " + ref }, 400);
        const cat = await catalogueMemo(produit, o);
        const cl = classe(cat.items, ref);
        return json({ produit, nom: PRODUITS[produit].nom, source: cat.source,
          repli: cat.repli || null, alertes: cat.alertes || [],
          reference: cl.reference, recupereA: isoSec(Date.now()),
          courante: itemPublic(cl.courante),
          futures: cl.futures.map(itemPublic),
          perimees: cl.perimees.map(itemPublic),
          trous: cl.trous });
      }
      if (url.pathname === "/api/meteo/pdf") {
        const produit = url.searchParams.get("product") || "";
        if (!PRODUITS[produit]) return json({ erreur: "produit inconnu : " + produit,
          produits: Object.keys(PRODUITS) }, 400);
        const cat = await catalogueMemo(produit, o);
        let validAt = url.searchParams.get("valid") || "";
        let statut = "demandée";
        if (validAt) {
          validAt = isoSec(Date.parse(validAt));
          if (!cat.items.some((x) => x.validAt === validAt)) {
            return json({ erreur: "échéance absente du catalogue : " + validAt,
              disponibles: cat.items.map((x) => x.validAt) }, 404);
          }
        } else {
          /* la même règle que l'application : en vigueur, sinon la prochaine
             publiée, sinon la dernière connue — jamais « rien » quand il y a
             quelque chose, et le statut le dit */
          const ref = url.searchParams.get("reference") || new Date().toISOString();
          if (!isFinite(Date.parse(ref))) return json({ erreur: "reference invalide : " + ref }, 400);
          const cl = classe(cat.items, ref);
          const it = cl.courante || cl.futures[0] || cat.items[cat.items.length - 1] || null;
          if (!it) return json({ erreur: "catalogue vide pour " + produit }, 404);
          validAt = it.validAt;
          statut = cl.courante ? "en vigueur" : (cl.futures[0] ? "à venir" : "dernière connue");
        }
        const d = await pdf(produit, validAt, Object.assign({ catalogue: cat }, o));
        const compact = validAt.replace(/[-:TZ]/g, "").slice(0, 12);
        return new Response(d.donnees, { status: 200,
          headers: Object.assign({
            "Content-Type": d.contentType || "application/pdf",
            "Cache-Control": "private, max-age=300",
            "Content-Disposition": "inline; filename=\"" + produit + "-" + compact + ".pdf\"",
            "Cross-Origin-Resource-Policy": "cross-origin",
            "Timing-Allow-Origin": "*",
            "X-Worker-Version": VERSION_W,
            "X-Meteo-Valide": validAt,
            "X-Meteo-Statut": statut,
            "X-Meteo-Source": d.source,
            /* les en-têtes que le relais LOGNAVAK sert déjà : l'application
               peut lire celui-ci comme l'autre */
            "X-Cartes-Date": compact + "00",
          }, ENTETES_CORS) });
      }
      return json({ service: "LOGNAVAK météo via SOFIA", version: VERSION_W,
        produits: Object.keys(PRODUITS),
        points: ["/api/meteo/catalog?product=…[&reference=ISO]",
                 "/api/meteo/pdf?product=…[&valid=ISO][&reference=ISO]"] }, url.pathname === "/" ? 200 : 404);
    } catch (e) {
      /* les messages du client sont déjà masqués (jamais de login) */
      return json({ erreur: String((e && e.message) || e).slice(0, 300),
        essais: (e && e.essais) || undefined }, 502);
    }
  },
};
