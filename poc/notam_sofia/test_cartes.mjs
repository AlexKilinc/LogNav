/* test_cartes.mjs — éprouve le FICHIER COMPLET qui sera collé dans Supabase.
   ...
   charge cartes_7.42.ts EN ENTIER, récupère le vrai gestionnaire passé à
...
const src = fs.readFileSync("cartes_7.42.ts", "utf8");
dit(src.includes('const VERSION = "7.42";'), "le fichier porte la version 7.42");
...
dit(v.http === 200 && v.corps.version === "7.42",
    "?version=1 répond 7.42 · " + v.corps.version);
dit(s.http === 200 && s.corps.ok === true,
    "?sofia=1&route=LFPN,LFPZ aboutit · HTTP " + s.http);
dit(s.corps.total === 7 && s.corps.nbSofia === 7,
    "7 NOTAM extraits, le compte de SOFIA concorde · " + s.corps.total + "/" + s.corps.nbSofia);
dit(s.corps.traduits === 5, "5 sont traduits en français · " + s.corps.traduits + "/7");
...
dit(vide.version === "7.42", "et l’en-tête de version suit partout · " + vide.version);
